...
from sklearn.metrics import accuracy_score, balanced_accuracy_score, f1_score, confusion_matrix,multilabel_confusion_matrix,ConfusionMatrixDisplay
import time
import os
import copy,csv
import statistics
...
for fold, (train_idx,val_idx) in enumerate(splits.split(dataset)):
    ...
    fscore_std.append(fold_results["Fold_"+str(fold+1)]["best_fscore"].item())
...
print("Standard deviation of fscore validatiob{}".format(fscore_std))

### initialise the final model with the pretrained model structure###
final_model = models.googlenet()
...
save_path_final = f'./googlenet_roaddata_bceloss-{k}Folds{num_epochs}epochs.pth'
